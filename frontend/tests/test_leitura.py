...
def test_create_leitura(login, page, supabase_admin, test_user_id,
    ...
    page.locator("select[name='cilindro_id']").first.select_option(str(cil_id))
    page.locator("select[name='elemento_id']").first.select_option(str(el_id))
    page.locator("input[name='quantidade']").first.fill("3")
    ...
    result = supabase_admin.table("leitura").select("id,quantidade").eq("cilindro_id", cil_id).eq("elemento_id", el_id).execute()
    assert result.data, "Leitura não foi persistida no banco"
    assert result.data[0]["quantidade"] == 3

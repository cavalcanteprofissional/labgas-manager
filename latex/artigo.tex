...
\textbf{Leituras} & Registro de análises vinculando cilindro, elemento, data, tempo de chama e quantidade \\
\textbf{Amostras} & Registro de amostras com associação N:N a elementos, lote e número manual \\
...
\subsubsection{Abas com Controle de Permissão:}
...
\midrule
\textbf{Cilindros} & Cadastro e controle de cilindros de gás com código único (formato CIL-XXX), status e consumo \\
...
\subsubsection{Áreas de Acesso Livre:}
...
\textbf{Nota:} A aba Admin é acessível exclusivamente para usuários com perfil de administrador ou dev (\texttt{is\_admin()}), sendo controlada pela verificação de role, não pelas permissões de abas.
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.9\textwidth]{figuras/navegacao_abas}
...
\item Usuários \textbf{admin} ou \textbf{dev} têm acesso irrestrito a todas as abas, incluindo o painel Admin
\item O menu lateral só exibe a opção Admin para usuários com \texttt{is\_admin()} (admin ou dev)
\end{enumerate}
...
\subsubsection{Sistema de Controle de Permissões}
...
\begin{enumerate}
\item \textbf{Acesso padrão (default):} Todas as 6 abas controladas são liberadas automaticamente para novos usuários (\texttt{true} por padrão)
...
\item \textbf{Herança administrativa:} Usuários com perfil de administrador (\texttt{role = 'admin'} ou \texttt{'dev'}) têm acesso completo a todas as abas, independente das permissões configuradas. A verificação \texttt{is\_admin()} retorna \texttt{True} para ambos antes de verificar as permissões individuais.
...
\midrule
\texttt{cilindro} & Registro de cilindros de gás com código, data de compra, consumo, custo e status \\
\texttt{elemento} & Catálogo de elementos analisados com consumo em L/min \\
\texttt{leitura} & Registro de análises vinculando cilindro, elemento, data, tempo de chama e quantidade \\
\texttt{amostra} & Registro de amostras com associação N:N a elementos, lote e número manual \\
\texttt{amostra\_elemento} & Tabela pivô da relação N:N entre amostras e elementos \\
\texttt{pressao} & Medições de pressão e temperatura vinculadas a cilindro \\
\texttt{perfil} & Perfis de usuários com role (dev/admin/usuario), status e permissões por módulo \\
...
\subsubsection{Índices do Banco de Dados}
...
\midrule
cilindro & idx\_cilindro\_user\_id & user\_id \\
cilindro & idx\_cilindro\_codigo & codigo \\
elemento & idx\_elemento\_user\_id & user\_id \\
elemento & idx\_elemento\_nome & nome \\
leitura & idx\_leitura\_user\_id & user\_id \\
leitura & idx\_leitura\_cilindro\_id & cilindro\_id \\
leitura & idx\_leitura\_elemento\_id & elemento\_id \\
leitura & idx\_leitura\_data & data \\
amostra & idx\_amostra\_user\_id & user\_id \\
amostra & idx\_amostra\_lote & lote \\
amostra & idx\_amostra\_lote\_created & lote, created\_at DESC \\
amostra\_elemento & idx\_amostra\_elemento\_amostra\_id & amostra\_id \\
amostra\_elemento & idx\_amostra\_elemento\_elemento\_id & elemento\_id \\
pressao & idx\_pressao\_user\_id & user\_id \\
pressao & idx\_pressao\_cilindro\_id & cilindro\_id \\
pressao & idx\_pressao\_data & data \\
historico\_log & idx\_historico\_log\_user\_id & user\_id \\
...
leitura & Público & Próprio usuário & Próprio usuário \\
amostra & Público & Próprio usuário & Próprio usuário \\
amostra\_elemento & Público & Próprio usuário & Próprio usuário \\
...
\begin{enumerate}
\item \textbf{Hierarquia de Roles (3 níveis)} — \texttt{dev} (super-admin, bypass RLS), \texttt{admin} (gestão de usuários + exportação, respeita RLS nos dados), \texttt{usuario} (acesso apenas aos próprios dados)
...
\subsubsection{1. Leituras por Cilindro}

\textbf{Descrição:} Total de leituras processadas por cada cilindro, indicando o consumo relativo de cada unidade.

\textbf{Cálculo:} Para cada registro de leitura, o sistema conta as leituras agrupando pelo \texttt{cilindro\_id}. O resultado é ordenado alfabeticamente pelo código do cilindro.

\subsubsection{2. Consumo por Elemento × Tempo de Chama}
...
\subsubsection{3. Elementos mais Analisados}

\textbf{Descrição:} Ranking de elementos por frequência de análise, orientando o planejamento de estoque.

\textbf{Cálculo:} O sistema agrupa as leituras por \texttt{elemento\_id}, soma a quantidade de leituras, e retorna o TOP 5 elementos com maior frequência de uso. Esta informação é crucial para previsões de estoque e planejamento de aquisição de gases.

\subsubsection{4. Eficiência de Cilindros por Elemento}

\textbf{Descrição:} Métrica que relaciona a quantidade de leituras processadas com o consumo de gás, permitindo identificar cilindro com melhor desempenho.

\textbf{Cálculo:} Esta métrica identifica quais combinações de cilindro × elemento produzem mais leituras, permitindo otimizar o uso dos recursos e identificar padrões de consumo.
